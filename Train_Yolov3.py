...
class AdamW(Adam):
    def __init__(self, weight_decay_rate=0.0001, *args, **kwargs):
    ...
    def get_updates(self, loss, params):
        ...
        for p in params:
            if p.name.endswith('kernel:0') or p.name.endswith('bias:0'):
                updates.append((p, p - self.lr * p * self.weight_decay_rate))
        return updates
...
class Train_Yolov3:
    ...
    def create_model(self,
        ...
        weight_decay = 0.001
        optimizer = AdamW(weight_decay_rate=weight_decay, learning_rate=lr)
        # optimizer = Adam(learning_rate=lr, clipnorm=0.001, weight_decay=0.001)
    ...
    def train(self):
        if FLAGS.save_location == "":
            ...
            temp_dir = os.path.join(self.saveLocation, "temp")
            os.makedirs(temp_dir, exist_ok=True)

            with tempfile.NamedTemporaryFile(delete=False, dir=temp_dir) as tmp:
                shutil.copy(config_path, tmp.name)
                os.rename(tmp.name, os.path.join(self.saveLocation, "config.json"))
