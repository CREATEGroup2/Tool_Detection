...
class BatchGenerator(Sequence):
    ...
    def random_brightness(self, image, delta=30):
        hsv_image = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
        h, s, v = cv2.split(hsv_image)
        v = np.clip(v + np.random.randint(-delta, delta), 0, 255).astype(np.uint8)
    ...
    def _aug_image(self, instance, net_h, net_w):
        ...
        im_sized = self.random_brightness(im_sized)
        ...
        rows, cols, _ = im_sized.shape
        M = cv2.getRotationMatrix2D((cols/2, rows/2), rotation_angle, 1)
        im_sized = cv2.warpAffine(im_sized, M, (cols, rows))
